//CALKONATOR ~~~projekt programistyczny przelasa~~~data rozpoczecia 15.11.2018
...
do {printf("\n\nCzy masz zainstalowany program gnuplot? [t/n]:   "); odpowiedz = getchar();} while (odpowiedz != 't' && odpowiedz != 'n');
if (odpowiedz == 'n')  system("sudo apt install gnuplot-x11");
...
fprintf(plik, "\n# W granicach [%Lf ; %Lf] \n\n\n # Proby			Wartosc\n", dolna, gorna); 	 //Przygotowanie wstepne pliku
...
for(z = 0; z< proby; )
		{
			wynik = 0;
			los = dolna + (((long double)rand()/(long double)RAND_MAX) * (gorna-dolna)) ; // SPRAWDZ POPRAWNOSC TEGO jesj ok 
